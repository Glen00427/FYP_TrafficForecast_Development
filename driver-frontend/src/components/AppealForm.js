...
export default function SideMenu({
  ...
  // read mirrored user from window (set by App)
  const appUser =
  ...
  async function loadRejectedAndAppeals() {
    ...
    // 1) Get my incidents then filter to "rejected"
    ...
    // 2) For those incidents, find latest appeal (any author)
    ...
    // keep newest-only per incident
    const latestMap = {};
    (aps || []).forEach((a) => {
      const ks = keyS(a.incident_id);
      if (!latestMap[ks]) {
        latestMap[ks] = a; // ordered newest-first
  ...
  // load once when menu opens (so badge is ready immediately)
  useEffect(() => {
    if (!open || !userId) return;
    loadRejectedAndAppeals();
  }, [open, userId]);

  // refresh when Inbox section is opened
  useEffect(() => {
  ...
  // Build display per your two cases:
  // Case 1: rejected incident with NO appeal
  // Case 2: rejected incident whose latest appeal.status === 'rejected'
  ...
  return (
    ...
    >
      <aside className="sm-panel sm-panel--new" onClick={(e) => e.stopPropagation()}>
          ...
          <>
            ...
            <nav className="sm-nav" aria-label="Navigation">
              ...
              >
                ...
                Inbox
                {!!inboxCount && (
                  <span className="sm-badge" aria-label={`${inboxCount} items`}>
                    {inboxCount}
                  </span>
                )}
              </button>
        ...
        {/* Appeal modal (opens only for Case 1) */}
        {Boolean(appealFor) && (
          <div
